...
function AadharModal() {
  const isOpen = useSelector((state) => state.aadharModalState.isOpen)
  ...
  const uploadAadhar = async () => {
    ...
    const imageRef = ref(
      storage,
      `images/${session.user._id}/profileImg/${session.user.email}/aadhar`
    )
